...
def create_er_targets_and_exits(world: "DeathsDoorWorld", entrances_to_randomize: list[tuple[DeathsDoorEntrance, DeathsDoorEntrance | None]]):
    for (A_to_B, B_to_A) in entrances_to_randomize:
        ...
        B_region = world.multiworld.get_region(
                A_to_B.ending_region.value, world.player
            )
...
def connect_plando(world: "DeathsDoorWorld", plando_connections: DeathsDoorPlandoConnections) -> None:
    ...
    for plando_connection in plando_connections:
        ...
        coupled = world.options.entrance_randomization == EntranceRandomization.option_coupled
        if plando_connection.direction != "both" and coupled and two_way:
            raise Warning(f"If entrance randomization is set to coupled, all two-way plando_connections will be forced to be both. Plando connection {plando_connection.entrance} to {plando_connection.exit} has direction {plando_connection.direction}")

        from_region = world.multiworld.get_region(
                entrance.starting_region.value, world.player
            )
        to_region = world.multiworld.get_region(
                exit.ending_region.value, world.player
            )
...
def find_scene_transition_plando(name: str) -> tuple[DeathsDoorEntrance, bool]:
    scene_transition_pair = next(pair for pair in two_way_scene_transitions if pair[0].name == name or pair[1].name == name)
    if scene_transition_pair[0].name == name:
        ...
        raise OptionError(f"Provided name of entrance or exit {name} in plando_connections is not valid.")
...
def randomize_one_ways(world: "DeathsDoorWorld", coupled: bool):
    ...
    for (entrance, exit) in zip(entrances, exits):
        connection = world.get_region(entrance[0].starting_region.value).connect(world.get_region(exit[0].ending_region.value))
        world.entrance_pairings[entrance[0].starting_region.value] = exit[0].ending_region.value
